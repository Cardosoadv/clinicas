...
export function PacientePicker({ value, displayName, onChange, error }: PacientePickerProps) {
  ...
  return (
    <div className="cliente-picker" ref={containerRef}>
      ...
      {showDropdown && (
        <div className="cliente-picker__dropdown" id="paciente-picker-listbox" role="listbox">
          ...
          {!isLoading &&
            results.map((paciente, index) => (
              <button
                ...
                type="button"
                className={`cliente-picker__option${selectedIndex === index ? ' cliente-picker__option--active' : ''}`}
